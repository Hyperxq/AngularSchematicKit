...
function addSymbolToNgModuleMetadata(source, ngModulePath, metadataField, symbolName, importPath = null, addToMetadataField = true) {
...
function addImportToModule(source, modulePath, classifiedName, importPath, addToMetadataField = true) {
    return addSymbolToNgModuleMetadata(source, modulePath, 'imports', classifiedName, importPath, addToMetadataField);
}
...
function getEnvironmentExportName(source) {
    ...
    allNodes
        .filter(ts.isImportDeclaration)
        .filter((declaration) => declaration.moduleSpecifier.kind === ts.SyntaxKind.StringLiteral && declaration.importClause !== undefined)
        .map((declaration) => declaration.importClause.getChildAt(0))
