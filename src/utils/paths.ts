...
export function setStructurePaths(structures: FolderStructure[], path: FolderPath): FolderStructure[] {
...
function makeStructurePaths(structure: FolderStructure): FolderStructure {
    ...
    structure.children.map((structureChild) => {
        structureChild.path = new FolderPath(structureChild.name, structure.path?.getPath() || '', structure.path);
